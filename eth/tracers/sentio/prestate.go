...
func (t *SentioPrestateTracer) CaptureFault(env *vm.EVM, pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, depth int, err error) {
}
...
func NewSentioPrestateTracer(ctx *tracers.Context, cfg json.RawMessage) (vm.Tracer, error) {
...
func (t *SentioPrestateTracer) CaptureEnd(output []byte, usedGas uint64, d time.Duration, err error) {
	...
	if t.env.Depth == 0 {
		t.CaptureTxEnd(usedGas)
		return
	}
}

// CaptureState implements the EVMLogger interface to trace a single step of VM execution.
func (t *SentioPrestateTracer) CaptureState(env *vm.EVM, pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, rData []byte, depth int, err error) {
	stack := scope.Stack
	stackData := stack.Data()
	stackLen := len(stackData)
	caller := scope.Contract.Address()
	switch {
	case stackLen >= 2 && op == vm.SHA3:
