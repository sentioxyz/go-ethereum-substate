...
func (t *SentioTracer) CaptureStart(env *vm.EVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
	log.Info("CaptureStart with " + strconv.Itoa(env.Depth))
	if env.Depth == 0 {
...
func (t *SentioTracer) CaptureEnd(output []byte, usedGas uint64, d time.Duration, err error) {
...
func (t *SentioTracer) CaptureState(env *vm.EVM, pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, rData []byte, depth int, err error) {
...
func (t *SentioTracer) CaptureFault(env *vm.EVM, pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, depth int, err error) {
}
...
func NewSentioTracer(ctx *tracers.Context, cfg json.RawMessage) (vm.Tracer, error) {
